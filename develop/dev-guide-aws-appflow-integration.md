...
6.  In the **Connect to TiDB-Connector** dialog, enter the connection properties of the TiDB cluster. If you use a TiDB Serverless cluster, you need to set the <strong>TLS</strong> option to `Yes`, which lets the TiDB connector use the TLS connection. Then, click <strong>Connect</strong>.
