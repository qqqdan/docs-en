...
# TiDB Lightning Backends {#tidb-lightning-backends}
...
The **Local-backend**: `tidb-lightning` first encodes data into key-value pairs, sorts and stores them in a local temporary directory, and *upload* these key-value pairs to each TiKV node <em>as SST files</em>. Then, TiKV ingests these <em>SST files</em> into the cluster. The implementation of Local-backend is the same with that of Importer-backend but does not rely on the external `tikv-importer` component.
...
## How to choose the backend modes {#how-to-choose-the-backend-modes}
...
## TiDB Lightning Local-backend {#tidb-lightning-local-backend}
...
### Deployment for TiDB-backend {#deployment-for-tidb-backend}
...
#### Configuration description and samples {#configuration-description-and-samples}
...
### Migrating from Loader to TiDB Lightning TiDB-backend {#migrating-from-loader-to-tidb-lightning-tidb-backend}
...
## TiDB Lightning Importer-backend {#tidb-lightning-importer-backend}

### Deployment for Importer-backend mode {#deployment-for-importer-backend-mode}
...
#### Step 3: Start <code>tikv-importer</code> {#step-3-start-code-tikv-importer-code}
...
#### Step 4: Start <code>tidb-lightning</code> {#step-4-start-code-tidb-lightning-code}
